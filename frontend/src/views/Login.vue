<template>
  <div class="login-shell">
    <div class="login-hero">
      <div class="brand">{{ 'StoreNet' }} 网页控制台</div>
      <p class="subtitle">登录后查看门店、上机状态和消费记录。</p>
      <ul class="bullets">
        <li>· 选择门店，自助开始计时</li>
        <li>· 查看费用，核对消费记录</li>
        <li>· 结账遇到问题时，请联系店长</li>
  ...
  methods: {
    async onSubmit() {
      if (!this.form.identifier || !this.form.password) {
        this.error = '请输入用户名/邮箱和密码'
        return
      }
      this.loading = true
      this.error = ''
      try {
        const res = await this.$api.post('/auth/login', this.form)
